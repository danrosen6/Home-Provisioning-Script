...
function Select-Applications {
    ...
    $availableApps = @{
        ...
        "12" = @{Key="terminal"; Name="Windows Terminal"; Default=$true}  # Added for Windows 11
        "13" = @{Key="powertoys"; Name="Microsoft PowerToys"; Default=$true}  # Added for Windows 11
    }
...
function Install-WindowsTerminal {
    ...
    choco install microsoft-windows-terminal -y --no-progress 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Log "Windows Terminal installed successfully via Chocolatey!" -Level INFO -ForegroundColor Green
        return $true
    } else {
        Write-Log "Chocolatey installation failed, trying Microsoft Store..." -Level WARNING -ForegroundColor Yellow
...
function Install-PowerToys {
    ...
    # Try to install via Chocolatey
    choco install powertoys -y --no-progress 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Log "Microsoft PowerToys installed successfully via Chocolatey!" -Level INFO -ForegroundColor Green
        return $true
    } else {
        Write-Log "Chocolatey installation failed, trying winget..." -Level WARNING -ForegroundColor Yellow
